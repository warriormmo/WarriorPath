<?php
  ...
  // Время активности пользователя (сек.)
  define('TIME_USER', 60);
  // Время жизни непринятого сражения (сек.)
  define('TIME_BATTLE', 60);

  // Подгружаем модуль с функциями
  require_once('func.php');
  db_open();
  // Проверяем авторизан-ли пользователь
  check_auth();
  // Открываем соединение с базой данных

  // Вычисляем время, чтобы показать время генерации странички
  ...
  $my = db_fetch();
  if (empty($my)) {
    goto_error('Пользователь не найден в базе!');
  }
