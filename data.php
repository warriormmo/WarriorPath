<?php
  ...
  if (($my['Hod'] == 0) && ($enemy['Hod'] == 0)) {
    ...
    if ($re_my_inic > $re_enemy_inic) {
      //смотрим кому повезло - тот и первый
    ...
    for ($i = 1; $i <= ($my['Obrazovanie'] * 3); $i  ) {
      db_query('UPDATE `Hend` SET `' . $i . '` = "' . $arr[$i] . '" WHERE id = ' . $my['id'] . ' LIMIT 1;');
    }
    ...
    for ($i = 1; $i <= 30; $i  ) {
      db_query('UPDATE `koloda` SET `' . $i . '` = "' . $arr[$i] . '" WHERE id = ' . $my['id'] . ' LIMIT 1;');
    }
    ...
    for ($i = 1; $i <= ($enemy['Obrazovanie'] * 3); $i  ) {
      db_query('UPDATE `Hend` SET `' . $i . '` = "' . $arr[$i] . '" WHERE id = ' . $enemy['id'] . ' LIMIT 1;');
    }
    $arr = array();
    $j = 1;
    for ($i = 1; $i <= 30; $i  ) {
    ...
    for ($i = 1; $i <= 30; $i  ) {
      db_query('UPDATE `koloda` SET `' . $i . '` = "' . $arr[$i] . '" WHERE id = ' . $enemy['id'] . ' LIMIT 1;');
    }
    //конец сортировки карт!!!
  }
  //ФАЗА 1
  ...
  if (($hod == 1) && ($my['Hod'] == 4)) {
    ...
    if (($myhod[0] != "") || ($enemyhod[0] != "")) {
      $log = $log . "<b>Первое действие</b> \n";
      $mov  ;
    }
