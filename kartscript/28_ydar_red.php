...
if ($battle['dist'] > 0) {
  ...
  $hitcount = $enemy['Lovkost'] + $enemy['schoolred'] + $enemy['moddeystvie']+$enemymods['nadeystvie'] + $enemy['modranenie'] + $enemymods['napopodanie'] + $enemymods['napopblgudaru'] + $enemymods['napopvgolovu'] - 2; //кол кубиков на попадание с учетом всех модов -3 от карты
  if ($hitcount > 0) {
  ...
  if ($hit < 3) {
    ...
    {
      $damagecount = $enemy['Sila'] + $enemymods['nadamage'] - round(($my['Sila'] / 2)) + $mymods['naarmor'] + $enemy['modranenie'] + $enemymods['napovvgolovu'] + $enemymods['napovblgudaru'] - 2; //расчет кол кубиков на повреждение
      if ($damagecount > 0) {
