<?php
  if ($battle['dist'] == 2) {
    ...
    $hitcount = $enemy['Lovkost'] + $maxschool + $enemy['moddeystvie'] + $enemy['modranenie'] + $enemymods['napopodanie'] + $dopmoddist + $enemymods['napopdaludaru'] + $enemymods['napopdaludarnogoy'] - 2;
    if ($hitcount > 0) {
    ...
    //условие вполнения удара
      ...
      if ($hit > $blok) {
        ...
        $damagecount = $enemy['Sila'] + $enemymods['nadamage'] - round(($my['Sila'] / 2)) + $mymods['naarmor'] + $enemy['modranenie'] + $enemymods['napovvtors'] + $enemymods['napovdaludaru'] + $enemymods['napovdaludarnogoy'] + 1;
        if ($damagecount > 0) {
