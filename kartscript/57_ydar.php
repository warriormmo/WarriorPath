<?php
  ...
  $hitcount = $enemy['Lovkost'] + $enemy['schoolred'] + $enemy['moddeystvie'] + $enemy['modranenie'] + $enemymods['napopodanie'] - 4;
  if ($hitcount > 0) {
  ...
  //условие вполнения удара
    ...
    if ($hit > $blok) {
      ...
      $damagecount = $enemy['Sila'] + $enemy['nadamage'] - round(($my['Sila'] / 2)) + $mymods['naarmor'] + $enemy['modranenie'] + 4;
      if ($damagecount > 0) {
      ...
      //контузия
      db_query('UPDATE `Hend` SET `1` = "" WHERE id = ' . $my['id'] . ' LIMIT 1;');
  ...
  include("delmods.php");
  //занесение в моды
  db_query('INSERT INTO `modificators`(id,time,napopodanie)  VALUES (' . $enemy['id'] . ',1000,-1)');
