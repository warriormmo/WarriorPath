...
$hitcount=$enemy['Lovkost']+$maxschool+$enemy['moddeystvie']+$enemy['modranenie']+$enemymods['napopodanie']-1;//кол кубиков на попадание с учетом всех модов -3 от карты
...
if ($hit<3){$log =$log.$first." не в состоянии выполнить удар и теряет равновесие".$second.". \n";$enemy['moddeystvie']--;}//условие вполнения удара
else { // описание последствий удара
$log =$log.$first." бьет в бок ".$second.". \n";
...
{
$damagecount=$enemy['Sila']+$enemy['nadamage']-round(($my['Sila']/2))+$mymods['naarmor']+$enemy['modranenie'];//расчет кол кубиков на повреждение
...
}else{$log =$log.$second." отбил удар, а ".$first." теряет равновесие \n";$enemy['moddeystvie']--;}
