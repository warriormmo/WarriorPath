<?php
  ...
  if ($schoolred > 0) {
    //карта требует красную школу
    //вытераем предыдущую стойку
    db_query('DELETE FROM `modificators` WHERE (`id`=' . $id . ') and (`stoyka`=1)');
    //занесение в базу в стойку
    db_query('UPDATE `game_user` SET `stoyka` = "40_mgnoven" WHERE id = ' . $id . ' LIMIT 1;');
    //занесение в моды
    db_query('INSERT INTO `modificators`(id,time,nadamage,napopodanie,nayklonenie,nablok,stoyka)  VALUES (' . $id . ',1000,-2,-3,3,3,1)');
  } else {
