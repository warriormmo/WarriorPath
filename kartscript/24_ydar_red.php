...
if ($battle['dist'] == 2) {
  ...
  $hitcount = $enemy['Lovkost'] + $enemy['schoolred'] + $enemy['moddeystvie']+$enemymods['nadeystvie'] + $enemy['modranenie'] + $enemymods['napopodanie'] + $dopmoddist + $enemymods['napopdaludaru'] + $enemymods['napopdaludarrukoy'] + $enemymods['napopvtors'] + 1; //кол кубиков на попадание с учетом всех модов -3 от карты
  if ($hitcount > 0) {
  ...
  if ($hit < 3) {
    ...
    {
      $damagecount = $enemy['Sila'] + $enemymods['nadamage'] - round(($my['Sila'] / 2)) + $mymods['naarmor'] + $enemy['modranenie'] + $enemymods['napovvtors'] + $enemymods['napovdaludaru'] + $enemymods['napovdaludarrukoy']; //расчет кол кубиков на повреждение
      if ($damagecount > 0) {
